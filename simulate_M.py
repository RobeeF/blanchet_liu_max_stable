...
def compute_theta(Ao=0, LAMBDA=1, gamma=.5, nb_iter=100, epsilon=10**(-6)): # Not working
...
def conditioned_sampleX(a,n, cov): 
    ...
    Y =  simul_Xk(cov) # Why don't we take X_{K-1} to start the sequence ? Maybe because of the independence of the X_{n}s

    return Y - cov[:,tv]*Y[tv]/cov[tv,tv] + X_tv # Returns X_k


def sample_single_record(a,ni,n_i_plus_1, cov):  # Does not depend on delta.. (Only through a)
    ...
    X = simul_Xk(cov=cov, size=K-1) # Compute X_1,...,X_{K-1}
    X_K  = conditioned_sampleX(a,n_i_plus_1+K,cov) # Compute X_K
    ...
    cond2 = u*compute_go_x(K,a,ni,bar_sigma) < theoretical_proba/empirical_proba # Not sure for cond2 that just inversing the ratio is ok
...
def sample_without_recordX(a, ni,l, cov): # Typo in the paper ? 
    ''' Sample X_{N_X+1}, ..., X_N
    - a (float in (0,1)): Parameter of the model obtained from compute_a(.)
    ...
    while (nl.norm(X, ord=np.inf,axis=1) - a*np.log(ni+np.arange(1,l+1))>0).any(): # Typo for the infinite norm 
        X = simul_Xk(cov=cov, size=l) # Why don't we start from X_{ni-1}, does not break the dynamic ? 
    return X
...
def algorithm_M(a, cov, gamma=.5, return_rnd_times=False):
    ...
    d = cov.shape[0]
    X = algorithm_X_NX(a, cov).reshape(-1,d) # Simulate X_1,...,X_NX
